...
static int get_cert_location(char *           uri,
                             int              uri_len,
                             DWORD *          store_name,
                             wchar_t **       store_path,
                             CRYPT_HASH_BLOB *thumbprint)
{
	...
	*sep = 0;
	...
	if (!CryptStringToBinaryA(thumbprint_str,
...
#define OPEN_PRIVATE_STORE_ERROR(__err) \
	{ \
		ret = __err; \
		goto open_private_key_from_store_done; \
	}

static int open_private_key_from_store(unsigned char *     key,
                                       int                 key_len,
                                       ...
                                       NCRYPT_KEY_HANDLE * hKey_out)
{
	...
	*hKey_out         = (NCRYPT_KEY_HANDLE)NULL;
	...
	if (!(hCertStore = CertOpenStore(CERT_STORE_PROV_SYSTEM,
	                                 ...
	                                 store_name | CERT_STORE_OPEN_EXISTING_FLAG |
	...
	if (NCryptOpenStorageProvider(&hStorageProv, pKeyProvInfo->pwszProvName, 0) !=
	    ERROR_SUCCESS)
		OPEN_PRIVATE_STORE_ERROR(ENOENT);

	dwKeyFlags =
	    NCRYPT_SILENT_FLAG |
	    ((store_name == CERT_SYSTEM_STORE_LOCAL_MACHINE) ? NCRYPT_MACHINE_KEY_FLAG
	                                                     : 0);
	if (NCryptOpenKey(hStorageProv,
	                  &hKey,
...
#define OPEN_PUBLIC_STORE_ERROR(__err) \
	{ \
		ret = __err; \
		goto open_public_key_from_store_done; \
	}

static int open_public_key_from_store(unsigned char *    key,
                                      int                key_len,
                                      HCERTSTORE *       hCertStore_out,
                                      PCCERT_CONTEXT *   pSignerCert_out,
...
#define OPEN_PUBLIC_PEM_ERROR(__err) \
	{ \
		ret = __err; \
		goto open_public_key_from_pem_done; \
	}

static int open_public_key_from_pem(unsigned char *    key,
                                    int                key_len,
                                    BCRYPT_KEY_HANDLE *hKey_out)
{
	int                   ret            = EINVAL;
...
#define SIGN_HMAC_ERROR(__err) \
	{ \
		ret = __err; \
		goto jwt_sign_sha_hmac_done; \
	}
                      ...
                      const char *  str)
{
	...
	if (BCryptHashData(hHash, (PUCHAR)str, (ULONG)strlen(str), 0) !=
	    ERROR_SUCCESS)
		SIGN_HMAC_ERROR(EINVAL);

	if (BCryptFinishHash(hHash, pbHashValue, cbHashValue, 0) != ERROR_SUCCESS)
		SIGN_HMAC_ERROR(EINVAL);
...
#define VERIFY_HMAC_ERROR(__err) \
	{ \
		ret = __err; \
...
int jwt_verify_sha_hmac(jwt_t *jwt, const char *head, const char *sig)
{
	...
	/* Encode as Base64. */
	if (!CryptBinaryToStringA(pbHash,
	                          cbHash,
...
int jwt_sign_sha_pem(jwt_t *jwt, char **out, unsigned int *len, const char *str)
{
	 ...
	 */
	ret = open_private_key_from_store(jwt->key,
	                                  jwt->key_len,
	                                  &hCertStore,
	                                  &pSignerCert,
	                                  &hStorageProv,
	...
	if (BCryptOpenAlgorithmProvider(&hAlg, alg, NULL, 0) != ERROR_SUCCESS)
		SIGN_PEM_ERROR(EINVAL);
...
int jwt_verify_sha_pem(jwt_t *jwt, const char *head, const char *sig_b64)
{
	...
	if (is_public_key_pem(jwt->key, jwt->key_len)) {
		ret = open_public_key_from_pem(jwt->key, jwt->key_len, &hKey);
	} else {
		ret = open_public_key_from_store(jwt->key,
		                                 jwt->key_len,
		                                 &hCertStore,
		                                 &pSignerCert,
		                                 &hKey);
	...
	/* Hash data. */
	if (BCryptOpenAlgorithmProvider(&hHashAlg, alg, NULL, 0) != ERROR_SUCCESS)
		VERIFY_PEM_ERROR(EINVAL);

	if (BCryptGetProperty(hHashAlg,
	                      BCRYPT_OBJECT_LENGTH,
	                      (PBYTE)&cbHashObject,
	                      sizeof(DWORD),
	                      &cbDummy,
	...
	if (BCryptHashData(hHash, (PUCHAR)head, (ULONG)strlen(head), 0) !=
	    ERROR_SUCCESS)
		VERIFY_PEM_ERROR(EINVAL);
